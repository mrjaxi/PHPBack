	<div class="col-md-9">
		...
		<div>
			<h5 style="color:#2C3E50;"><?php echo $type->name; ?></h5>
			<span style="color:#34495E"><small><?php echo $type->description; ?></small></span>
		</div>
		<table id="ideastable" class="table table-condensed">
		<thead>
			<tr>
			  <th><small><?php echo $lang['label_idea']; ?> <a id="order-by--idea" href="<?= $type->url .'/title/'; echo ($type == 'desc') ? 'asc' : 'desc';?>"><span class="glyphicon glyphicon-chevron-<?php echo ($type == 'desc') ? 'down' : 'up'; ?>" style="margin-left:4px"></span></a></small></th>
			  <th><small><?php echo $lang['label_votes']; ?> <a id="order-by--votes" href="<?= $type->url .'/votes/'; echo ($type == 'desc') ? 'asc' : 'desc';?>"><span class="glyphicon glyphicon-chevron-<?php echo ($type == 'desc') ? 'down' : 'up'; ?>" style="margin-left:4px"></span></a></small></th>
			  <th><small><?php echo $lang['label_comments']; ?></small></th>
			  <th><small><?php echo $lang['label_date']; ?> <a id="order-by--date" href="<?= $type->url .'/id/'; echo ($type == 'desc') ? 'asc' : 'desc';?>"><span class="glyphicon glyphicon-chevron-<?php echo ($type == 'desc') ? 'down' : 'up'; ?>" style="margin-left:4px"></span></a></small></th>
			</tr>
		</thead>
		</table>
		...
		<?php endforeach; ?>
			<ul class="pagination">
			  <li><a class="pagination--previous" href="<?php if($page > 1) echo $type->url . "/$order/$type/" . ($page-1); else echo '#'; ?>">&laquo;</a></li>
			  <?php for($i=1;$i<=$pages;$i++){ ?>
			  	 <?php if($i == $page): ?>
			  	 	<li class="active"><a class="pagination--current" href=""><?php echo $i;?></a></li>
			  	 <?php else:?>
			  	 	<li><a class="pagination--page" href='<?php echo $type->url . "/$order/$type/$i"; ?>'><?php echo $i;?></a></li>
			  	 <?php endif;?>

			  <?php } ?>
			  <li><a class="pagination--next" href="<?php  if($page < $pages) echo $type->url . "/$order/$type/" . ($page+1); else echo '#'; ?>">&raquo;</a></li>
			</ul>
</div>
